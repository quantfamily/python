...
class Database:
    ...
    def stock_data(self, isin: str = None):
        if isin:
            query = f"""Select isin, time, high, low, open, close, volume
                        FROM ohlc INNER JOIN instrument ON ohlc.isin = instrument.isin
                        WHERE ohlc.time BETWEEN '{self.date_manager.start}' AND '{self.date_manager.current}' AND
                        instrument.isin='{isin}'"""
        else:
            query = f"""Select isin, time, high, low, open, close, volume
                        FROM ohlc WHERE ohlc.time
                        BETWEEN '{self.date_manager.start}' AND '{self.date_manager.current}'"""
    ...
    def get_position(self, instrument: Instrument) -> Position:
        ...
        with self.session_maker() as db_session:
            ...
            query = query.filter(Instrument.isin == instrument.isin)
